 ...
 */
async function processQueue(sheetsClientEmail, sheetsPrivateKey, sheetsSheetId, valuationKey, creatomateApiKey, creatomateTemplateId, publerApiKey, publerProfileIds) {
    var _a;
    const sheets = await getSheetsClient(sheetsClientEmail.value(), sheetsPrivateKey.value());
    ...
    const rows = (_a = readRes.data.values) !== null && _a !== void 0 ? _a : [];
    ...
    for (const row of pending) {
        try {
            ...
            // 1. Valuate the plate
            const valuationRes = await axios_1.default.get("https://us-central1-code-g-b8b6f.cloudfunctions.net/valuePlate", { params: { plate: row.plate, key: valuationKey.value() } });
            ...
            const renderRes = await axios_1.default.post("https://api.creatomate.com/v1/renders", {
            ...
            for (let i = 0; i < 24; i++) {
                ...
                const statusRes = await axios_1.default.get(`https://api.creatomate.com/v1/renders/${renderId}`, {
                    headers: {
                        Authorization: `Bearer ${creatomateApiKey.value()}`,
                    },
                });
            ...
            await axios_1.default.post("https://api.publer.io/v1/posts", {
